...
def get_status(Account_number):
    ...
    test2 = f"select case when exists ( select user from rec_auth where user = '{Account_number}') then cast (1 as bit) else cast (0 as BIT) end"
...
#         statusreturn = mycursor.fetchone()
    ...
    if var[0]:
        ...
        #if exist return the user_id of the given row from within the rec_auth table
